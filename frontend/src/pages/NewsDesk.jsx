  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center gap-3 border-b border-border pb-4">
        <Newspaper className="text-neon-cyan" size={28} />
        <div>
      ...
      {news.length === 0 ? (
        ...
        >
          {news.map((item) => (
            <motion.div key={item.id} variants={itemAnim} className="bg-surface border border-border rounded-xl p-6 space-y-4 hover:border-teal-900/50 transition-colors">
              ...
              <div>
                <p className="text-sm font-mono text-text-muted mb-2 border-l-2 border-border pl-3">
                  "{item.headline}"
                </p>
              ...
              <div className="bg-amber-950/10 border border-amber-900/30 rounded-lg p-4 flex gap-3 text-sm">
                <Lightbulb className="text-neon-gold shrink-0 mt-0.5" size={18} />
                <div>
                  <span className="font-bold text-neon-gold block mb-1">Beginner Tip</span>
                  <p className="text-slate-300">{item.beginner_tip}</p>
                </div>
              </div>
            </motion.div>
          ))}
